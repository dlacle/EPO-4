...
def start_pairing(carrier_frequency, bit_frequency, comport='COM7'):
    ...
    # Carrier freq
    carrier_freq_bytes = (carrier_frequency).to_bytes(2, byteorder='big')
    serial_port.write(b'F' + carrier_freq_bytes + b'\n')
    time.sleep(0.1)

    # Bit freq
    bit_freq_bytes = (bit_frequency).to_bytes(2, byteorder='big')
    serial_port.write(b'B' + bit_freq_bytes + b'\n')
    time.sleep(0.1)
