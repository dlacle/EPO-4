...
# File path mic data
location_car = '80x400'
file_path_mic = f'Mic-Data/kitt_carrier_2250_bit_3k_{location_car}.txt'
...
# set threshold for find first peak channel response
threshold = 0.25
...
def plot_each_segment_and_each_channel_separately(segment, Fs, location_car, n, lowest_peak_value):
    time = np.linspace((lowest_peak_value[n] - 950) / Fs, (lowest_peak_value[n] - 950) / Fs + len(segment[n * 5]) / Fs,
                       len(segment[n * 5]))
...
def plotting_channel_response_of_every_channel_in_one_plot_each_segment(n, lowest_peak_value, h1, h2, h3, h4, h5,
                                                                        peak_location):
...
def automatically_segment(peak_filter, lst):
    ...
    for p in range(len(peak_filter[0])):
        ...
        print(
            f"lowest peak of each segment = {lowest_peak}, {lowest_peak / Fs}sec, came from sublist [{sublist_index}]")
...
def find_peaks(h1, h2, h3, h4, h5, treshold):
    ...
    print(f'The peak locations are: {peak_location}\n')
...
def difference_peaks(location_peak):
    ...
    print(f'differences between each pair of elements: {diff_peak}')
...
print('\nLocalization result:\n', localization(data_recording, xref, mic_positions_xyz, Fs, eps, Vsound, len(xref), location_car, threshold))
