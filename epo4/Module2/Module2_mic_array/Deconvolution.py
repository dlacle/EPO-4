...
# File path mic data
file_path_mic = r"C:\Users\Sam\PycharmProjects\EPO-4\epo4\Module2\Module2_mic_array\Mic-Data\Mic-Data-V1\kitt_carrier_2250_bit_3k_305x160.txt"
...
def automatically_segment(peak_filter, lst):
    ...
    for p in range(len(peak_filter[0])):
        ...
        i = min(sublist[p] for sublist in peak_filter if p < len(sublist))  # extra cond.
    ...
    for p in range(len(peak_filter[0])):
        ...
        lowest_peak = min(sublist[p] for sublist in peak_filter if p < len(sublist))  # extra cond
