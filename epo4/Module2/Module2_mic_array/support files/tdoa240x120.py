...
def plotting(x, y, Fs):
    ...
    t_start =  2.4  # Start time (seconds)
    t_end =  2.7 # End time (seconds)
    ...
    plt.title(f'KITT recording 240x120')
    plt.show()
...
def TDOA(x, y, Fs):
    ...
    t_start = 2.4  # Start time (seconds)#2.03 - 2.20
    t_end = 2.7  # End time (seconds)
    ...
    y_ch1 = y[4:len(y):5][start_index:end_index]
...
def main():
    ...
    Fs = 48e3
    x = np.loadtxt('../Mic-Data/Mic-Data-V1/ref_sig_V1.5.txt')
    y = np.loadtxt('../Mic-Data/Mic-Data-V1/kitt_carrier_2250_bit_3k_240x120.txt')
