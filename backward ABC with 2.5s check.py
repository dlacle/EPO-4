...
x2 = np.array([int(input('x2: ', )) / 100, int(input('y2: ', )) / 100])  # [x2, y2] end location
...
while dx > 0.001:
    ...
    if x0[0] < 0.25 or x0[0] > 4.55 or x0[1] < 0.25 or x0[1] > 4.55:
        ...
        # kitt.drive(150, 150)
        dt, t, v, x0, d0, alpha = measure_loc(t, v, m, 150, L, alpha, 155, x0, Fb_max)
        power = 158  # backward
    ...
    if dx < 0.1 and np.any(x_target != x2) or t > 2.5:
        print('x1 reached')
        # kitt.brake(v)
        # kitt.stop()
        power = 150
